    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper bg-red-50 ">
            ...
            if (isset($_GET['masuasp'])) {
                $masuasp = $_GET['masuasp'];
            }
            $sql = "select*from products where product_id=$masuasp";
            $kq = $conn->query($sql)->fetch();
$dm = $kq['category_id'];
            ?>
            <style>
            ...
            <div class="card-body pb-16 container mx-auto">
                <h2 class="text-center text-xl font-bold my-10">SỬA SẢN PHẨM</h2>
                <div class="border mx-auto bg-white p-4" style=" width: 30%">
                    <form action="" class="mx-auto" method="POST" enctype="multipart/form-data">
                        ...
                        <table class="mx-auto p-10 text-lg" style="width: 50%;">
                            ...
                            <tr>
                                <td> <input class=" border p-1 rounded-lg " width="100%" placeholder="Tên sản phẩm..." type="text" value="<?php echo $kq['product_name'] ?>" name="tensp" id=""> </td>
                            </tr>

                            <tr>
                                <th class="pt-3">Giá sản phẩm</th>
                            </tr>
                            <tr>
                                <td>
                                    <input class=" border p-1 rounded-lg " width="500px" type="number" placeholder="Giá gốc..." value="<?php echo $kq['product_price'] ?>" name="giagoc" id="">
                                </td>
                            </tr>
                            <!-- 1hang -->

                            <tr>
                                <th class="pt-3">Giá sale</th>
                            </tr>
                            <tr>
                                <td>
                                    <input class=" border p-1 rounded-lg " width="30px" type="number" placeholder="Giá sale..." value="<?php echo $kq['product_sale'] ?>" name="giasale" id="">
                                </td>
                            </tr>
                            <!-- 1hang -->

                            <tr>
                                <th class="pt-3">Ảnh sản phẩm</th>
                            </tr>
                            <tr>

                                <td><img src="img/<?php echo $kq['product_image'] ?>" alt=""></td>
                            ...
                            <tr>
                                <th class="pt-3">Chi tiết</th>
                            </tr>
                            <tr>
                                <td>
                                    <textarea class="border w-56 rounded-lg p-1" style="height: 100px; width: 230px;" name="chitiet" value="<?php echo $kq['product_detail'] ?>" id="" placeholder="Chi tiết sản phẩm..." cols="30" rows="10"><?php echo $kq['product_detail'] ?>u</textarea>
                                </td>
                            </tr>
                            <!-- 1hang -->
                            <tr>
                                <td> <input class="px-3 py-2 font-bold text-lg hover:bg-yellow-600 bg-yellow-400 rounded-lg" type="submit" name="btn_sua" value="Sửa" id=""></td>
...
if (isset($_POST['btn_sua'])) {
    ...
    if (empty($name_anh)) {
        $name_anh = $kq['product_image'];
    }
    move_uploaded_file($_FILES['anhsp']['tmp_name'], "img/" . $name_anh);
    $sql = "update products set product_name='$tensp',product_price='$giagoc',product_sale='$giasale',product_image='$name_anh',product_detail='$chitiet' where product_id=$masuasp";
    $kq = $conn->prepare($sql);
    if ($kq->execute()) {
        echo "Thanh cong";
        header("location:qtsanpham.php?madmshow=$dm");
    } else {
        echo "loi";
    };
}
